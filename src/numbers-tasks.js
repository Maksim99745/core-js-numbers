...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  if (Number.isFinite(x1) === false || Number.isFinite(x2) === false) {
    return 1.7976931348623157e308;
  }
  if (Number.isFinite(y1) === false || Number.isFinite(y2) === false) {
    return 1.7976931348623157e308;
  }
  const x = (x2 - x1) ** 2;
  const y = (y2 - y1) ** 2;
  ...
  if (Number.isFinite(x + y) === false) {
    return 1.7976931348623157e308;
  }
  const result = Math.sqrt(x + y);
  if (Number.isFinite(result) === false || result > Number.MAX_VALUE) {
    return 1.7976931348623157e308;
  }
  return result;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  return Math.abs(Math.atan2(y1 * x2 - x1 * y2, x1 * x2 + y1 * y2));
}
...
function toNumber(value, def) {
  const test = typeof Number(value);
  if (test === 'number' && !Number.isNaN(Number(value))) {
    return Number(value);
  }
  return def;
}
...
function getSumOfDigits(num) {
  ...
  const sum = strArr.reduce((acc, current) => acc + Number(current), 0);
  return sum;
}
...
function isNumber(number) {
  if (Number.isNaN(number * 1) || !Number.isFinite(number)) {
    return false;
  }
...
function getHypotenuse(a, b) {
  const lenghts = Math.hypot(a, b);
  if (lenghts < Number.MAX_VALUE) {
    return lenghts;
  }
  return 1.7976931348623157e308;
}
